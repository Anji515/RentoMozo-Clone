...
import { Grid,GridItem,Text,Image,Heading,Button,Input,Spinner,Select, Flex, Box, Stack, SimpleGrid, Center } from '@chakra-ui/react';
...
const handleWish=(item)=>{
 ...
 const updatedWish = cart.map((wish)=>{
   ...
   if(!exist){
    ...
    useEffect(()=>{
        let apiUrl = getUrl(`https://royalrento.onrender.com/fitness?_page=${page}&_limit=8`,sort,orderBy,searchQuery )
        fetchdata(apiUrl)
    ...
    return loading ? (<Spinner thickness='5px' speed='0.75s' emptyColor='gray.200' color='blue.500' width={'250px'}
        height='250px' />) : (<div gap='10px'>
            <br />
        <Heading fontSize={'24px'} color='teal'>Welcome To Fitness Equipements Page</Heading>   
        <br />
        <Center >
        ...
        {data?.map((el)=>( 
           <GridItem boxShadow= 'rgba(0, 0, 0, 0.35) 0px 5px 15px' borderRadius={'25px'} key={el.id} padding='10px'>
            <Goto to={`/productFitness/${el.id}`} ><Image src={el.imageUrl} borderTopRadius={'25px'} height={'250px'} width='100%' /></Goto>
